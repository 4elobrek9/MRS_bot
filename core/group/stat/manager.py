        ...
        if profile_data:    
            ...
            if last_activity_date != current_date:
                ...
                if last_activity_date:
                    ...
                    if last_activity_date == yesterday:
                        cursor = await self._conn.execute(
                            'SELECT COUNT(*) FROM user_activity_log WHERE user_id = ? AND date = ? AND exp_gained > 0',
                            (user_id, yesterday)
                        )
                        had_exp_yesterday = (await cursor.fetchone())[0] > 0
                        if not had_exp_yesterday:
                            flames = 0
            ...
            await self._conn.execute(
                '''UPDATE user_profiles 
                   ...
                   WHERE user_id = ?''',
                (new_level, exp, lumcoins, daily_messages, total_messages, flames, current_date, user_id)
            )
            ...
            await self._conn.execute(
                '''INSERT INTO user_profiles 
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                (user_id, 1, ProfileConfig.EXP_PER_MESSAGE_INTERVAL, 0, 1, 1, 1, 0, 'default', current_date)
            )
